...
from src.train_qot_model import train_model, load_network_data, load_training_data, prepare_training_data
from src.eon_control import EONController
from src.generate_synthetic_data import generate_data, generate_training_samples, save_data
from src.lightpath_reader import LightpathReader
from src.ml_qot import MLQoTEstimator, QoTFeatures
import networkx as nx
from src.process_lightpath_data import process_lightpath_dataset
...
def add_required_attributes(G: nx.Graph) -> nx.Graph:
    ...
    for node in G.nodes():
        ...
        G.nodes[node]['has_regeneration'] = True
...
def convert_lightpath_data_to_qot_features(lightpath_data: List[Tuple], G: nx.Graph) -> List[Tuple[QoTFeatures, float]]:
    ...
    for link, qot_value in lightpath_data:
        ...
        # Calculate realistic features based on physical parameters
        link_length = link.length
        fiber_loss = link_length * 0.2  # 0.2 dB/km
        splice_loss = 0.1 * (link_length / 2)  # 0.1 dB per splice, one every 2 km
        ...
        features = QoTFeatures(
            ...
            num_amplifiers=max(0, int(np.ceil(link_length / 80))),  # Corrected method: no amps for < 80km
            total_loss=total_loss,  # Realistic fiber + splice loss
