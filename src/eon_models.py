...
class EONNode:
    ...
    def __init__(self,
        ...
        self.has_regeneration = has_regeneration
        self.num_add_drop_ports = num_add_drop_ports
        self.connected_links: Dict[str, 'EONLink'] = {}
        self.regenerators: List[TransceiverSpec] = []
        
    def add_link(self, link: 'EONLink', direction: str) -> None:
        """Add a link to the node"""
        self.connected_links[direction] = link
        
    def get_available_regenerators(self) -> int:
        """Get number of available regenerators"""
        return len(self.regenerators)
        
    def allocate_regenerator(self, spec: TransceiverSpec) -> bool:
        """Allocate a regenerator if available"""
        if not self.has_regeneration or not self.regenerators:
            return False
        self.regenerators.pop()
        return True
...
class EONLink:
    ...
    def calculate_osnr(self, launch_power: float, num_amplifiers: int) -> float:
        ...
        bandwidth_hz = 12.5 * 1e9  # 12.5 GHz channel

        # Span and amplifier setup
        span_length_km = 80  # Standard span length
        num_spans = int(np.ceil(self.length / span_length_km))
