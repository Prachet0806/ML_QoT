...
class SpectrumManager:
    def __init__(self, graph, total_slots=320):
        ...
        # Caching mechanism for fragmentation metrics
        self._fragmentation_cache = {}
        self._cache_valid = False
        self._current_request_id = None

    def _invalidate_cache(self):
        """Invalidate the fragmentation cache when network state changes."""
        self._fragmentation_cache.clear()
        self._cache_valid = False
        
    def _get_cache_key(self, edge: Optional[Tuple[str, str]], spectrum_data: Optional[np.ndarray] = None) -> str:
        """Generate a cache key for fragmentation calculations."""
        if spectrum_data is not None:
            # For spectrum-specific calculations, use hash of spectrum data
            return f"spectrum_{hashlib.md5(spectrum_data.tobytes()).hexdigest()}"
        elif edge is not None:
            # For edge-specific calculations, use edge identifier
            return f"edge_{edge[0]}_{edge[1]}"
        else:
            # Fallback for edge-specific calculations without edge parameter
            return "edge_default"
    ...
    def _calculate_fragmentation(self, edge: Tuple[str, str]) -> float:
        """Calculate fragmentation score for an edge with caching."""
        edge = (str(edge[0]), str(edge[1]))  # Ensure tuple of exactly two strings
        
        # Check cache first
        cache_key = self._get_cache_key(edge)
        if cache_key in self._fragmentation_cache and self._cache_valid:
            return self._fragmentation_cache[cache_key]
        ...
        if len(occupied_slots) == 0:
            ...
            if len(gaps) == 0:
                ...
                result = float(std_gap / (mean_gap + 1e-6))
        
        # Cache the result
        self._fragmentation_cache[cache_key] = result
        return result

    def _calculate_fragmentation_metrics(self, spectrum: np.ndarray) -> dict:
        """Calculate fragmentation metrics for a spectrum array with caching."""
        # Check cache first
        cache_key = self._get_cache_key(None, spectrum)
        if cache_key in self._fragmentation_cache and self._cache_valid:
            return self._fragmentation_cache[cache_key]
    ...
    def start_request_processing(self, request_id: str):
        """Start processing a new request - enables caching for this request."""
        self._current_request_id = request_id
        self._cache_valid = True
        self._fragmentation_cache.clear()  # Clear old cache
        
    def end_request_processing(self):
        """End processing the current request - invalidates cache."""
        self._cache_valid = False
        self._fragmentation_cache.clear()
        self._current_request_id = None
    ...
    def allocate_spectrum(self, path: List[str], bandwidth: float) -> Optional[List[int]]:
        ...
        # Invalidate cache since spectrum state changed
